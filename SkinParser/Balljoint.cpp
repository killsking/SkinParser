...
void Balljoint::calculate_joints_lcs(double *parentMatrix, int &joint_id) {
    ...
    //save joint's lcs into joints_lcs, increment joint id
    for (int i = 0; i < 16; i++) {
...
map<int, vector<float>> Balljoint::getJointsLcs() {
    //indicate the base's initial transform as identity and id as zero
...
void Balljoint::getPose(float *pose, int joint_id) {
    if (this->joint_id == joint_id) {
        for (int i = 0; i < 3; i++) {
            pose[i] = this->pose[i];
        }
...
void Balljoint::setPose(float *pose, int joint_id) {
    if (this->joint_id == joint_id) {
        for (int i = 0; i < 3; i++) {
            this->pose[i] = pose[i];
        }
